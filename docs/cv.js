...
// Chatroom IA
let startNewConversationBtn, generateChatCvSummaryBtn, conversationList,
...
const showPage = (pageId) => {
    ...
    if (activeSection) {
        ...
        if (pageId === 'dashboard') {
            ...
            downloadCvBtn.style.display = 'none'; // Cacher le bouton de téléchargement par défaut
            editCvBtn.style.display = 'none'; // Cacher le bouton d'édition par default
            valorizeCvContentBtn.disabled = true; // Désactiver la valorisation par défaut
            ...
            // Ensure initial state for chat input/buttons
            // These elements *must* be defined by now since DOMContentLoaded has fired
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (clearCvInputBtn) {
        clearCvInputBtn.addEventListener('click', () => {
            ...
            editCvBtn.style.display = 'none'; // Cacher le bouton d'édition
            valorizeCvContentBtn.disabled = true; // Désactiver jusqu'à ce qu'un CV soit généré
    ...
    // Initialize chat pagination controls
    initPaginationControls(fetchConversations, selectConversation, deleteConversation);
...
async function fetchDashboardInsights() {
    ...
    try {
        ...
        totalEstimatedCostUSDEl.textContent = `${(insights.totalEstimatedCostUSD ?? 0).toFixed(2)}`;
        totalEstimatedCostEUREl.textContent = `${(insights.totalEstimatedCostEUR ?? 0).toFixed(2)}`;
...
async function generateCv() {
    ...
    editCvBtn.disabled = true; // Désactiver le bouton d'édition pendant la génération
    downloadCvBtn.style.display = 'none';
    ...
    try {
        ...
        editCvBtn.style.display = 'inline-flex'; // Activer le bouton d'édition
        valorizeCvContentBtn.disabled = false; // Activer la valorisation
        ...
        valorizationOutput.innerHTML = `<p class="placeholder-text error-message">Veuillez d'abord générer un CV valide pour la valorisation.</p>`;
    } finally {
        generateCvBtn.disabled = false;
        clearCvInputBtn.disabled = false;
        editCvBtn.disabled = false; // Réactiver le bouton d'édition
    }
}
...
async function loadLastStructuredCvData() {
    try {
        ...
        if (response.ok) {
            ...
            cvInput.value = JSON.stringify(data, null, 2); // Afficher le JSON dans l'input pour référence
 ...
 */
function downloadCv() {
    const cvHtmlContent = cvOutput.innerHTML;
...
async function valorizeCvContent() {
    const contentToValorize = cvInput.value.trim(); // Use the raw input as the source for valorization
    ...
    valorizeCvContentBtn.disabled = true;
    valorizationOutput.innerHTML = '<p class="placeholder-text">Analyse et valorisation en cours...</p>';
...
async function fetchConversations(page = 1) {
    ...
    try {
        ...
        renderChatConversationList(selectConversation, deleteConversation);
        ...
        if (conversationList) conversationList.innerHTML = '<p class="placeholder-text error-message">Erreur de chargement des conversations.</p>';
...
async function deleteConversation(id) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await fetchConversations(currentChatPage); // Re-fetch to update list
            await fetchDashboardInsights(); // Refresh dashboard
            // Ensure pagination controls are updated after deletion
            renderChatConversationList(selectConversation, deleteConversation);
        } else {
            const data = await response.json();
            if (response.status === 429) {
