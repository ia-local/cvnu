...
async function fetchAiDetails() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            outputDiv.textContent = `Erreur: Impossible de générer l'explication. ${data.message || 'Erreur inconnue'}`;
        }
...
async function startGanDebate() {
    ...
    try {
        ...
        // Mettre à jour les stats après un débat
        fetchFaqStats(); 
        refreshEpochStatus();
...
async function refreshEpochStatus() {
    ...
    try {
        ...
        // CORRECTION DE L'AFFICHAGE DU COMPTEUR NÉGATIF/INVERSÉ
...
async function generate1001Solutions() {
    ...
    try {
        ...
        let htmlContent = `
            ...
            <div style="display: display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 10px;">
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Liaison des événements aux boutons (Vérification de l'existence avant d'attacher)
    
    // Simuler l'impact CVNU
    const simulateBtn = document.querySelector('button[onclick="simulateActionImpact()"]');
    if (simulateBtn) simulateBtn.addEventListener('click', simulateActionImpact);

    // Débat GAN
    const ganBtn = document.querySelector('button[onclick="startGanDebate()"]');
    if (ganBtn) ganBtn.addEventListener('click', startGanDebate);
    
    // 1001 Solutions
    const btn1001 = document.querySelector('button[onclick="generate1001Solutions()"]');
    if (btn1001) btn1001.addEventListener('click', generate1001Solutions);
    
    // FAQ Stats
    const fetchBtn = document.getElementById('fetch-faq-btn');
    if (fetchBtn) fetchBtn.addEventListener('click', fetchFaqStats); 

    // Ajout FAQ Suggérée (bouton qui apparaît après l'époque)
    const addFaqBtn = document.getElementById('add-faq-btn');
    if (addFaqBtn) addFaqBtn.addEventListener('click', addSuggestedFaq);
    
    // Appel initial au chargement des données
    fetchFaqStats(); 
    refreshEpochStatus(); 
});