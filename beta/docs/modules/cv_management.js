// docs/modules/cv_management.js

// Importations corrigées
import { getApiBaseUrl, showStatusMessage, getCurrentCvStructuredData, setCurrentCvStructuredData } from '../app.js';
...
export function initCvManagementPage() { // Removed explicit parameters, now uses imports
    ...
    if (clearCvInputBtn) {
        clearCvInputBtn.onclick = () => {
            ...
            setCurrentCvStructuredData(null); // Use global setter
        };
    }
    if (downloadCvBtn) {
...
async function generateCv() {
    ...
    const currentData = getCurrentCvStructuredData(); // Use global getter
    if (!rawCvContent && !currentData) {
    ...
    try {
        let cvDataToRender = currentData;
        ...
        if (rawCvContent) {
            ...
            setCurrentCvStructuredData(cvDataToRender); // Update global state
        } else if (!currentData) {
...
async function loadLastStructuredCvData() {
    try {
        ...
        if (response.ok) {
            ...
            setCurrentCvStructuredData(data); // Use global setter
...
async function openCvEditorModal() {
    const currentCvStructuredData = getCurrentCvStructuredData(); // Use global getter
    ...
    if (result) {
        ...
        setCurrentCvStructuredData(updatedData); // Use global setter
