...
import { getApiBaseUrl, showStatusMessage } from '../app.js'; // Import global functions
...
export function initDashboardPage() {
    ...
    totalTaxeIAAmountEl = document.getElementById('totalTaxeIAAmount'); // NOUVEAU
    ...
    thematicUtmiEducationTrainingEl = document.getElementById('thematicUtmiEducationTraining'); // NOUVEAU
    thematicUtmiProfessionalDevelopmentEl = document.getElementById('thematicUtmiProfessionalDevelopment'); // NOUVEAU
...
export async function fetchDashboardInsights() {
    ...
    try {
        ...
        updateElementText(totalTaxeIAAmountEl, (insights.totalTaxeIAAmount ?? 0).toFixed(2)); // NOUVEAU
        ...
        renderObjectList(utmiPerCostRatioByModelEl, insights.utmiPerCostRatioByModel || {}, (key, value) => `<strong>${key}:</strong> ${(value ?? 0).toFixed(2)}`); // FIX HERE toFixed on client side
        ...
        updateElementText(thematicUtmiEducationTrainingEl, (insights.thematicUtmi?.educationTraining ?? 0).toFixed(2)); // NOUVEAU
        updateElementText(thematicUtmiProfessionalDevelopmentEl, (insights.thematicUtmi?.professionalDevelopment ?? 0).toFixed(2)); // NOUVEAU
        ...
        const elementsToUpdate = [
            totalUtmiEl, totalEstimatedCostUSDEl, totalEstimatedCostEUREl, totalTaxeIAAmountEl, // NOUVEAU
            totalInteractionCountEl, averageUtmiPerInteractionEl, totalUtmiPerCostRatioEl,
            thematicUtmiMarketingEl, thematicUtmiAffiliationEl, thematicUtmiFiscalEconomicEl,
            thematicUtmiEducationTrainingEl, thematicUtmiProfessionalDevelopmentEl // NOUVEAU
        ];
...
function renderDashboardChart() {
    ...
    if (ctx) {
        ...
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Total UTMi', 'Coût USD', 'Coût EUR', 'Taxe IA'], // NOUVEAU: Ajout Taxe IA
                datasets: [{
                    label: 'Values',
                    data: [
                        ...
                        parseFloat(totalTaxeIAAmountEl.textContent) // NOUVEAU
                    ],
                    backgroundColor: [
                        'rgba(54, 162, 235, 0.6)',
                        'rgba(255, 159, 64, 0.6)',
                        'rgba(75, 192, 192, 0.6)',
                        'rgba(255, 99, 132, 0.6)' // NOUVEAU: Couleur pour la Taxe IA (ex: rouge)
                    ],
                    borderColor: [
                        'rgba(54, 162, 235, 1)',
                        'rgba(255, 159, 64, 1)',
