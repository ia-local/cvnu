// public/app.js - Logique UI Complète, Pagination & Modale
...
const appPages = document.querySelectorAll('.app-page'); // Toutes les sections de page
...
const totalEstimatedCostEUREl = document.getElementById('totalEstimatedCostEUR'); // Added back
...
const cvAnalysisOutput = document.getElementById('cv-analysis-output'); // Placeholder for future semantic analysis
const analyzeCvValueBtn = document.getElementById('analyzeCvValueBtn'); // Placeholder for future semantic analysis

// --- Chatbot Modal Elements ---
const chatbotModal = document.getElementById('chatbotModal');
const closeModalBtn = chatbotModal.querySelector('.close-modal-btn'); // Using querySelector for specific modal context
const startNewConversationBtn = document.getElementById('startNewConversationBtn'); // Re-declared for modal context
 ...
 * @param {string} type - 'alert' (OK button) or 'confirm' (Confirm/Cancel buttons).
 * @returns {Promise<boolean>} Resolves to true for confirm, false for cancel.
 */
function showModal(title, bodyHtml, type = 'alert') {
    return new Promise(resolve => {
        const modalElement = document.getElementById('appModal'); // Use a generic appModal if you have one, or redefine for this context
        const modalTitleEl = modalElement.querySelector('#modalTitle');
        const modalBodyEl = modalElement.querySelector('#modalBody');
        const modalFooterEl = modalElement.querySelector('#modalFooter');
        const modalConfirmBtn = modalElement.querySelector('#modalConfirmBtn');
        const modalCancelBtn = modalElement.querySelector('#modalCancelBtn');
        const modalOkBtn = modalElement.querySelector('#modalOkBtn');
        ...
        modalFooterEl.style.display = 'flex'; // Ensure footer is visible if buttons are needed

        // Event listeners (re-attached each time to prevent multiple triggers)
        const confirmHandler = () => { modalElement.classList.remove('active'); resolve(true); removeListeners(); };
        const cancelHandler = () => { modalElement.classList.remove('active'); resolve(false); removeListeners(); };
        const okHandler = () => { modalElement.classList.remove('active'); resolve(undefined); removeListeners(); }; // Undefined for alert
        ...
        const removeListeners = () => {
            ...
            // Assuming `closeModalBtn` for generic modal is separate from chatbot modal's
            // If it's a generic one, attach `okHandler` or `cancelHandler` based on type
        };

        const clickOutsideHandler = (event) => {
            if (event.target === modalElement) {
                if (type === 'confirm') cancelHandler();
                else okHandler();
            }
        };
        ...
        if (type === 'confirm') {
            ...
            // closeModalBtn is for the chatbot modal, not this generic one in the provided HTML.
            // If a generic close button is needed on the showModal, ensure it's in index.html for appModal.
...
// fetchDashboardInsights function already exists and moved here
...
async function fetchDashboardInsights() {
    ...
    try {
        ...
        totalEstimatedCostEUREl.textContent = insights.totalEstimatedCostEUR.toFixed(2); // Ensure this is present
    ...
    conversationsToDisplay.forEach(conv => {
        ...
        li.innerHTML = `
            <div>
                ...
                <small>UTMi: ${conv.utmi_total ? conv.utmi_total.toFixed(2) : '0.00'} EUR</small>
            </div>
        `;
...
async function sendMessage() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            await fetchChatConversations(); // Update list in modal
