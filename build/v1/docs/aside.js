...
// --- Dynamic Left Navigation Elements ---
const dynamicLeftNav = document.getElementById('dynamic-left-nav'); // Nouveau conteneur de nav dynamique
const dynamicNavList = document.getElementById('dynamic-nav-list'); // Liste des liens dynamiques
...
function showPage(pageId) {
    ...
    // Set active navigation link in the main navbar
    ...
    // Removed sidebarNavLinks as it's now dynamically rendered
    ...
    // Render the dynamic left navigation based on the active page
    renderDynamicLeftNav(pageId);
}

/**
 * Renders the dynamic left-side navigation based on the current active page/thematic.
 * @param {string} pageId - The ID of the currently active page.
 */
function renderDynamicLeftNav(pageId) {
    dynamicNavList.innerHTML = ''; // Clear previous content

    let navContent = [];

    switch (pageId) {
        case 'home':
            dynamicLeftNav.style.display = 'flex'; // Show for desktop
            navContent = [
                { id: 'home-overview', label: 'Vue d\'ensemble', icon: 'fas fa-chart-pie' },
                { id: 'home-onboarding', label: 'Démarrage Rapide', icon: 'fas fa-play-circle' },
                { id: 'home-faq', label: 'FAQ', icon: 'fas fa-question-circle' },
            ];
            break;
        case 'dashboard':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'dashboard-metrics', label: 'Statistiques Clés', icon: 'fas fa-tachometer-alt' },
                { id: 'dashboard-details', label: 'Détails UTMi', icon: 'fas fa-info-circle' },
                { id: 'dashboard-reports', label: 'Rapports Personnalisés', icon: 'fas fa-file-invoice' },
            ];
            break;
        case 'user-profile':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'profile-cv-gen', label: 'Générateur CV', icon: 'fas fa-file-alt' },
                { id: 'profile-valorization', label: 'Valorisation Compétences', icon: 'fas fa-award' },
                { id: 'profile-settings', label: 'Paramètres Profil', icon: 'fas fa-cog' },
            ];
            break;
        case 'chatroom':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'chat-conversations', label: 'Mes Conversations', icon: 'fas fa-list' },
                { id: 'chat-new-conv', label: 'Nouvelle Discussion', icon: 'fas fa-plus-circle' },
                { id: 'chat-summary', label: 'Résumé de Chat', icon: 'fas fa-file-word' },
            ];
            break;
        case 'workspace':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'workspace-projects', label: 'Projets', icon: 'fas fa-project-diagram' },
                { id: 'workspace-tools', label: 'Outils', icon: 'fas fa-tools' },
                { id: 'workspace-settings', label: 'Préférences', icon: 'fas fa-sliders-h' },
            ];
            break;
        case 'documentation':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'doc-guide', label: 'Guide Utilisation', icon: 'fas fa-book-reader' },
                { id: 'doc-api', label: 'Référence API', icon: 'fas fa-code' },
                { id: 'doc-faq', label: 'FAQ Documentation', icon: 'fas fa-question' },
            ];
            break;
        case 'contact':
            dynamicLeftNav.style.display = 'flex';
            navContent = [
                { id: 'contact-form', label: 'Formulaire Contact', icon: 'fas fa-paper-plane' },
                { id: 'contact-info', label: 'Infos Contact', icon: 'fas fa-address-book' },
                { id: 'contact-support', label: 'Support Technique', icon: 'fas fa-headset' },
            ];
            break;
        default:
            dynamicLeftNav.style.display = 'none'; // Hide if no dynamic nav for this page
            break;
    }
    ...
    if (navContent.length > 0) {
        navContent.forEach(item => {
            ...
            li.innerHTML = `<a href="#${item.id}" class="dynamic-nav-link" data-sub-page="${item.id}">
                                <i class="${item.icon}"></i> ${item.label}
                            </a>`;
            // Add a click listener for dynamic sub-page navigation (you'll implement the actual sub-page content logic later)
            li.querySelector('a').addEventListener('click', (e) => {
                e.preventDefault();
                console.log(`Navigating to dynamic sub-page: ${item.id} within ${pageId}`);
                // Here, you would implement logic to show/hide sub-sections within the current main page.
                // For now, it's just a console log and visual active state.
                dynamicNavList.querySelectorAll('.dynamic-nav-link').forEach(link => {
                    link.classList.remove('active');
                });
                e.target.classList.add('active');
                showGlobalStatus(`Sous-section "${item.label}" chargée !`, 'info', 1000);
            });
            dynamicNavList.appendChild(li);
        });
        // Set the first item as active by default if there are items
        ...
        dynamicNavList.innerHTML = '<li class="placeholder-text">Pas de sous-sections pour cette page.</li>';
    }

    // Hide dynamic nav on mobile for all cases
    if (window.innerWidth < 768) { // Adjust breakpoint if needed
        dynamicLeftNav.style.display = 'none';
    }
}
